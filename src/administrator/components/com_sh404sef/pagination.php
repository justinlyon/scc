<?php
/**
 * @version		$Id: pagination.php 1120 2010-01-10 22:32:44Z silianacom-svn $
 ...
 * 
 * Modified nov. 2009 - (c) Yannick Gaultier
 * 
...
class JPagination extends JObject
{
	...
	function _buildDataObject()
	{
		...
		if ($this->get('pages.current') > 1)
		{
			...
			$data->previous->link	= JRoute::_("&limitstart=".$page."&limit=".$this->limit);
		}
		...
		if ($this->get('pages.current') < $this->get('pages.total'))
		{
			...
			$data->next->base	= $next;
			$data->next->link	= JRoute::_("&limitstart=".$next."&limit=".$this->limit);
			$data->end->base	= $end;
			$data->end->link	= JRoute::_("&limitstart=".$end."&limit=".$this->limit);
		}
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
		{
			...
			if ($i != $this->get('pages.current') || $this->_viewall)
			{
				...
				$data->pages[$i]->link	= JRoute::_("&limitstart=".$offset."&limit=".$this->limit);
			}
		}
		return $data;
	}

...
class SeriesPage extends MasterPage
{	
	/**
	 ...
	 */
	 private function renderSummaryPageModel($model)
	 {
		...
		foreach( $model->getList() as $cat ) {
			...
			$tmpl->addVars('series', BeanUtil::beanToArray($cat,true)); // scalars only	
			
			$tmpl->addVar('series','iter',$i);
			$tmpl->addVar('series','index',$i+1);
			
			// publication state
			$pubwidget = $this->getPubControls($cat->getPubState());
			$logger->debug("Publication state of category ". $cat->getName() .": ". $cat->getPubState());
			$tmpl->addVars('series',$pubwidget);
			
			// number of events
			$events = $cat->getEvents();
			$logger->debug("Number of events". count($events));
			$tmpl->addVar('series','program',count($events));
			
			$tmpl->parseTemplate('series','a');
			$i++;
		}
		
		$tmpl->displayParsedTemplate( 'series_summary' );
	 }
	 ...
	 */
	 private function renderDetailPageModel($model)
	 {
		...
		$tmpl = $this->createPatTemplate();
		$tmpl->readTemplatesFromInput( 'series_detail.pat.tpl' );
		
		// Render the primary form fields
		$tmpl->addVars('series_detail', BeanUtil::beanToArray($cat,true)); // scalars only
		
		// Render the editor field
		$dconf = $this->getEditorConfig(Editor::MEDIUM, 'description', $cat->getDescription());
		$tmpl->addVar('series_detail','desc_editor',$this->setEditor($dconf));
		...
		$tmpl->displayParsedTemplate( 'series_detail' );

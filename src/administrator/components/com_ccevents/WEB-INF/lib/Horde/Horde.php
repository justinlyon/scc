 ...
 *
 * $Horde: framework/Horde/Horde.php,v 1.489.2.95 2008/06/13 04:52:33 slusarz Exp $
 */

/**
 * Log
 */
include_once 'Horde/Log.php';
    ...
    function fatal($error, $file, $line, $log = true)
    {
        ...
        if ($admin) {
            ...
            if (is_object($error)) {
                ...
                $errortext .= '<h4>' . _("The full error message is logged in Horde's log file, and is shown below only to administrators. Non-adminitrative users will not see error details.") . '</h4>';
...
HTML;
    ...
    function checkRequestToken($slug, $token)
    {
        ...
        if (isset($GLOBALS['conf']['token'])) {
            ...
            $tokenSource = Horde_Token::factory($GLOBALS['conf']['token']['driver'],
     ...
     *                                 file.
     * @oaram string $app              The application. Defaults to the current
     *                                 application.

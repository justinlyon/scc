<?php
/**
 * Series View for CCEvents Component
 * 
 * @package    joomla
 * @subpackage ccevents
 */
...
require_once(WEB_INF.DS.'pages'.DS.'FrontSeriesPage.php');
require_once(WEB_INF.DS.'beans'.DS.'Category.php');

class CCEventsSeriesViewSeries extends JView
{
   ...
   {   
      $heading = "Series at the Autry National Center";
   ...
   {   
      ...
      $overview = '';
      if ($model->getSeries() ) {
         $heading = $model->getSeries()->getName();
         $overview = $model->getSeries()->getDescription();   
      }
      $document =& JFactory::getDocument();
      $document->setTitle($heading);
      $this->assignRef('heading',$heading);
      ...
      $filter = "index.php?option=com_ccevents&scope=sers&task=summary";
      if ($_REQUEST['oid']) {
         $filter   = "&oid=". $_REQUEST['oid'];
      }
      if ($_REQUEST['fid']) {
      ...
      $fsp = new FrontSeriesPage();
      if (isset($_SESSION['summary_sort'])) {
         usort($events,array($fsp,'sort_by_' . $_SESSION['summary_sort']));
      } else {
         usort($events,array($fsp,'sort_by_date'));
      }
   ...
   {   
      ...
      $heading = "Series Detail: ". $event->getTitle();
      $document =& JFactory::getDocument();
      $document->setTitle($heading);
    ...
    * NOTE: This reimplements the FrontSeriesPage 
   ...
   function getTicketLink($code)
   {
      ...
      if ($code > 0) {
            $link = CCEventsSeriesViewSeries::TICKET_VENDOR_BASE . CCEventsSeriesViewSeries::TICKET_VENDOR_INT_QUERY . $code;
      } elseif (trim($code) != '') {
            $link = CCEventsSeriesViewSeries::TICKET_VENDOR_BASE . CCEventsSeriesViewSeries::TICKET_VENDOR_STRING_QUERY . $code;
      }
      return $link;   
   }
